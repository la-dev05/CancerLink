...
const Navbar = () => {
  ...
  // Flattened links for mobile menu
  const flatNavLinks = navCategories.flatMap((category) => category.links);
                ...
                <div className="ml-4 border-l pl-2">
                  {category.links.map((link) => (
                    <Link
                      key={link.name}
                      to={link.path}
                      className="flex items-center py-2 px-2 text-sm font-medium text-muted-foreground hover:text-foreground 
