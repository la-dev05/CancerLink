...
const survivors = [
  ...
  },
  {
    ...
    topics: ["Treatment Options", "Clinical Trials", "Side Effect Management", "Mental Health"]
  },
  {
    id: 3,
    ...
    topics: ["Young Adult Cancer", "Screening", "Family Support", "Life After Treatment"]
  },
  {
    id: 4,
    name: "David Williams",
  ...
  },
  {
    ...
    topics: ["Hormone Balance", "Recovery", "Lifestyle Changes", "Work-Life Balance"]
  },
  {
    id: 6,
    name: "Robert Garcia",
    ...
    topics: ["Young Adult Cancer", "Financial Navigation", "Healthcare Policy", "Survivorship"]
  }
];
      ...
      <CardHeader>
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <CardDescription>
              {survivor.cancerType} • {survivor.yearsAsSurvivor} {survivor.yearsAsSurvivor === 1 ? 'year' : 'years'} • {survivor.location}
            </CardDescription>
          </div>
        </div>
      </CardHeader>
    ...
    <Layout>
      <div className="py-12 md:py-16 container">
        <div className="max-w-4xl mx-auto text-center mb-12">
          <h1 className="text-3xl font-bold tracking-tighter mb-4">Connect with Cancer Survivors</h1>
          <p className="text-muted-foreground text-lg">
            Build meaningful connections with survivors who understand your journey and can provide guidance and support.
          </p>
        </div>
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative flex-grow">
              ...
              <Input placeholder="Search by name, cancer type, or location" className="pl-9" />
            </div>
          ...
          <Tabs defaultValue="all">
            ...
            <TabsContent value="breast" className="mt-0">
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {survivors.filter(s => s.cancerType === "Breast Cancer").map((survivor) => (
                  <SurvivorCard key={survivor.id} survivor={survivor} />
                ))}
              </div>
            </TabsContent>
            ...
            <TabsContent value="prostate" className="mt-0">
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {survivors.filter(s => s.cancerType === "Prostate Cancer").map((survivor) => (
                  <SurvivorCard key={survivor.id} survivor={survivor} />
                ))}
              </div>
            </TabsContent>
            ...
            <TabsContent value="colon" className="mt-0">
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {survivors.filter(s => s.cancerType === "Colon Cancer").map((survivor) => (
                  <SurvivorCard key={survivor.id} survivor={survivor} />
                ))}
              </div>
            </TabsContent>
            ...
            <TabsContent value="lung" className="mt-0">
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {survivors.filter(s => s.cancerType === "Lung Cancer").map((survivor) => (
                  <SurvivorCard key={survivor.id} survivor={survivor} />
                ))}
              </div>
            </TabsContent>
            ...
            <TabsContent value="other" className="mt-0">
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {survivors.filter(s => !["Breast Cancer", "Prostate Cancer", "Colon Cancer", "Lung Cancer"].includes(s.cancerType)).map((survivor) => (
                  <SurvivorCard key={survivor.id} survivor={survivor} />
                ))}
              </div>
            </TabsContent>
        ...
        <div className="mt-12 text-center">
          <p className="text-muted-foreground mb-4">
            Don't see what you're looking for? We're constantly adding new survivor partners.
          </p>
          <Button>
            Become a Survivor Volunteer
          </Button>
        </div>
      </div>
    </Layout>
